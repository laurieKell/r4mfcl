read.ini <- function (ini.file) 
{
  ...
  hpts <- grep("^#", a)
    ...
    pos <- grep("# tag fish rep group flags", a, ignore.case = T) + 
        1
    if (length(pos) > 0) {
        p2 <- hpts[hpts > pos][1] - 1
    ...
    pos <- grep("# tag_fish_rep active flags", a, ignore.case = T) + 
        1
    if (length(pos) > 0) {
        p2 <- hpts[hpts > pos][1] - 1
    ...
    pos <- grep("# tag_fish_rep target", a, ignore.case = T) + 
        1
    if (length(pos) > 0) {
        p2 <- hpts[hpts > pos][1] - 1
    ...
    pos <- grep("# tag_fish_rep penalty", a, ignore.case = T) + 
        1
    if (length(pos) > 0) {
        p2 <- hpts[hpts > pos][1] - 1
