    ...
    axis.lty = 0, offset = 0, scale = 1, ...) 
{
    ...
    if (is.vector(height) || (is.array(height) && (length(dim(height)) == 
        ...
        if (is.null(col)) 
            col <- grey(seq(0.3^2.2, 0.9^2.2, length = nrow(height))^(1/2.2))
    }
    ...
    if (plot) {
        ...
        xyrect <- function(x1, y1, x2, y2, horizontal = TRUE, scale,
            ...) {
            if (horizontal) 
                rect(x1, y1, scale*x2, y2, ...)
            else rect(y1, x1, y2, scale*x2, ...)
        }
        if (beside) 
            xyrect(0 + offset, w.l, c(height) + offset, w.r, 
                horizontal = horiz, scale, angle = angle, density = density, 
                col = col, border = border)
        else {
            for (i in 1:NC) {
                xyrect(height[1:NR, i] + offset[i], w.l[i], height[-1, 
                  i] + offset[i], w.r[i], horizontal = horiz, scale, 
